#!/bin/bash
# filepath: /C:/Users/lucio/programas/programas-python/microservicios/02-prueba/run.macos.sh
# Script para arrancar varios servidores FastAPI en diferentes puertos
# Uso: ./run.macos.sh [--dev] [--stop]
...
$UVICORN_CMD apis.server1.main:app --host 0.0.0.0 --port $PORT1 &
$UVICORN_CMD apis.server2.main:app --host 0.0.0.0 --port $PORT2 &
$UVICORN_CMD apis.server3.main:app --host 0.0.0.0 --port $PORT3 &
$UVICORN_CMD apis.server4.main:app --host 0.0.0.0 --port $PORT4 &
